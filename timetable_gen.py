...
def deterministic_schedule(teachers: list, existing_slots: list):
    ...
    base_occ_branch = set()   # (branch_lower,  day, timeslot) — cross-timetable

    for ex in existing_slots:
        for teacher in split_teachers(ex['teacher_name']):
            base_occ_t.add((teacher.lower(), ex['day'], ex['time_slot']))
        base_occ_branch.add((ex['branch_name'].strip().lower(),
                              ex['day'], ex['time_slot']))

    # ── Helpers ────────────────────────────────────────────────────────────

    def can_place(day, start_h, length, tname, subject, occ_t, occ_ts, occ_branch):
        """Return True iff this session can be placed at (day, start_h)."""
        ts_list = [fmt_slot(start_h + k) for k in range(length)]
        # Within-timetable: only one subject per (day, timeslot)
        if any((day, ts) in occ_ts for ts in ts_list):
            return False
        # Teacher availability — includes cross-timetable via base_occ_t
        if any((tc.lower(), day, ts) in occ_t
               for tc in split_teachers(tname)
               for ts in ts_list):
            return False
        # Branch availability — prevents same subject being taught twice
        # at the same time (cross-timetable included via base_occ_branch)
        if any((subject.lower(), day, ts) in occ_branch for ts in ts_list):
            return False
        return True
    ...
    def do_unplace(day, start_h, length, tname, subject,
        ...
        for ts in ts_list:
            ...
            occ_branch.discard((subject.lower(), day, ts))
    ...
    # Per-branch feasibility — a subject can't appear more times than available slots
    branch_external_hours = {}
    for ex in existing_slots:
        k = ex['branch_name'].strip().lower()
        branch_external_hours[k] = branch_external_hours.get(k, 0) + 1

    branch_needed_hours = {}
    for s in sessions:
        k = s['subject'].lower()
        branch_needed_hours[k] = branch_needed_hours.get(k, 0) + s['length']

    over_booked_branch = []
    for branch, needed in branch_needed_hours.items():
        external = branch_external_hours.get(branch, 0)
        available = avail_hours - external
        if needed > available:
            over_booked_branch.append(
                f"  Subject '{branch}' needs {needed}h but only "
                f"{available}h slots remain ({external}h taken by other timetables)"
            )
    if over_booked_branch:
        raise ValueError(
            "Schedule infeasible — subject(s) over-scheduled:\n"
            + "\n".join(over_booked_branch)
        )
    ...
    for trial in range(n_trials):
        ...
        occ_branch = set(base_occ_branch)
    ...
    def _domain(sess, ot, ots, ob):
        ...
        return [
            ...
            if can_place(day, sh, sess['length'], sess['teacher'],
                         sess['subject'], ot, ots, ob)
        ]

    def _forward_ok(from_idx, bt_ord, ot, ots, ob):
        for j in range(from_idx, len(bt_ord)):
            if not _domain(bt_ord[j], ot, ots, ob):
                return False
        return True
    ...
    for restart in range(BT_RESTARTS):
        ...
        def backtrack(idx):
            ...
            domain = _domain(sess, occ_t, occ_ts, occ_branch)
            if not domain:
                return False
            ...
            for day, sh in domain:
                ...
                if _forward_ok(idx + 1, bt_order, occ_t, occ_ts, occ_branch):
                    if backtrack(idx + 1):
                        return True
...
def check_internal_conflicts(slots: list, teachers: list,
    ...
    # Rule: one subject per (day, time_slot) within new timetable
    timeslot_occ = {}
    for s in slots:
        tk = (s['day'], s['time_slot'])
        if tk in timeslot_occ:
            existing = timeslot_occ[tk]
            if existing['subject_name'].lower() != s['subject_name'].lower():
                conflicts.append(
                    f"[TIME-CONFLICT] Two subjects at {s['day']} {s['time_slot']}: "
                    f"'{existing['subject_name']}' and '{s['subject_name']}'"
                )
        else:
            timeslot_occ[tk] = s
    ...
    for s in slots:
        ...
        if bk in branch_occ:
            conflicts.append(
                f"[INTERNAL-BRANCH] '{s['branch_name']}' double-booked "
                f"{s['day']} {s['time_slot']}"
            )
        else:
            branch_occ[bk] = True

    # Cross-timetable check
    ex_teacher_occ = {}
    ex_branch_occ  = {}
    for ex in existing_slots:
        for teacher in split_teachers(ex['teacher_name']):
            ex_teacher_occ[(teacher.lower(), ex['day'], ex['time_slot'])] = ex
        ex_branch_occ[(ex['branch_name'].lower(), ex['day'], ex['time_slot'])] = ex
    ...
    for s in slots:
        ...
        bkey = (s['branch_name'].lower(), s['day'], s['time_slot'])
        if bkey in ex_branch_occ:
            ex = ex_branch_occ[bkey]
            conflicts.append(
                f"[EXTERNAL-BRANCH] '{s['branch_name']}' already has a class "
                f"{s['day']} {s['time_slot']} "
                f"(Timetable #{ex.get('timetable_id', '?')})"
            )
